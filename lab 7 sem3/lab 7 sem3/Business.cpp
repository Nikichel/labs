...
void Business::filter() {			//фильт поиска
	...
	while (1) {
		cout << "Поиск по:" << endl << "1. Имя" << endl << "2. Фамилию" << endl << "3. Отчество" << endl << "4. Дату"
			<< endl << "5. Лицензию" << endl << "6. Паспорт" << endl << "7. Выйти из фильтра" << endl;				//выбор параметра
...
void Business::change() {			//меню изменения полей
	...
	while (fl) {
		cout << "Сменить" << endl << "1. Имя" << endl << "2. Фамилию" << endl << "3. Отчество" << endl
			<< "4. Паспорт" << endl << "5. Номер лицензии" << endl << "6. Добавить пересечение границы"
			<< endl << "7. Добавить выплату налога" << endl << "8. Выйти" << endl;
...
ifstream& operator>>(ifstream& in, Business& other) {
	in >> dynamic_cast<Human&>(other);		//вывод полей Human
	in.setf(ios::left);		//выравнивание по левому краю
	in >> setw(17)>> other.num_passport >> setw(7) >> other.addresLicense.city >> setw(10) >> other.addresLicense.street
		>> setw(8) >> other.addresLicense.home >> setw(17) >> other.license;
	for (int i = 0; i < count_ms; i++) {
		if (i == 0) {
			in >> other.taxes[i].data >> setw(9) >> other.taxes[i].summa;
			in >> other.broad_cross[i].data >> setw(12) >> other.broad_cross[i].country;
			in >> other.product[i];
			continue;
		}
		in >> other.taxes[i].data >> setw(9) >> other.taxes[i].summa;
		in >> other.broad_cross[i].data >> setw(12) >> other.broad_cross[i].country;
		in >> other.product[i];																	//вывод полей Business
	}
	in.unsetf(ios::left);
	return in;
};
ofstream& operator<<(ofstream& out, Business& other) {
	out << dynamic_cast<Human&>(other);		//вывод полей Human
	out.setf(ios::left);		//выравнивание по левому краю
	out << setw(5) << "" << setw(17) << other.num_passport
		<< setw(7) << other.addresLicense.city << setw(10) << other.addresLicense.street << setw(8) << other.addresLicense.home
