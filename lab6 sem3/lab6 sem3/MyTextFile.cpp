...
template <typename T>
void MyTextFile<T>::OpenRead() {			//открыть в режиме чтения
	try {
		this->fin.open(this->nameFile, ios::in);
		if (!this->fin.is_open()) 			//если файл не открылся/файла нет
			throw MyExceptionFILE(110, "Ошибка открытия файла", "Файла не существует");
	}
...
template <typename T>
void MyTextFile<T>::OpenWrite() {			//открыть в режиме записи
	try {
...
template <typename T>
void MyTextFile<T>::OpenAdd() {			//открыть в режиме добавить
	try {
