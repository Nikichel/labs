...
ifstream& operator>>(ifstream& in, Tourist& other) {
	in >> dynamic_cast<Human&>(other);			//вывод полей Human
	in.setf(ios::left);			//выравнивание по левому краю
	in >> setw(5) >> setw(17) >> other.num_passport;
	for (int i = 0; i < count_ms; i++) {
		if (i == 0) {
			in >> other.broad_cross[i].data >> setw(3) >> other.broad_cross[i].country;
			continue;
		}
		in >> other.broad_cross[i].data >> setw(3) >> other.broad_cross[i].country;			//вывод полей Tourist
	}
	in.unsetf(ios::left);
	return in;
}

ofstream& operator<<(ofstream& out, Tourist& other) {
	out << dynamic_cast<Human&>(other);			//вывод полей Human
	out.setf(ios::left);			//выравнивание по левому краю
	out << setw(5) << "" << setw(17) << other.num_passport;
	for (int i = 0; i < count_ms; i++) {
